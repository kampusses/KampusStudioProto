...
namespace KampusStudioProto.Areas.Identity.Pages.Account
{
    ...
    public class LoginWith2faModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            {
                _logger.LogWarning("Utente con ID '{UserId}' bloccato.", user.Id);
                return RedirectToPage("./Lockout");
            }
            else
            {
                _logger.LogWarning("Codice di autenticazione non valido immesso per l'utente con ID '{UserId}'.", user.Id);
